...
在开始任何操作前，先获取当前项目根目录，记为 `$PROJECT_ROOT`。后续所有路径均以此为基础进行替换拼接。

```powershell
$PROJECT_ROOT = (Get-Location).Path
```
将文档中所有包含 `$PROJECT_ROOT` 的路径替换为实际路径。
...
在 PowerShell 中执行以下命令，安装 `office-word-mcp-server` 包到 `$PROJECT_ROOT\.uv\tool` 目录：
...
每一次对话都是一个独立的机器学习实验任务。用户提供原始实验文件夹（如 `\第X次实验`），如果文件是压缩包，需要先解压。
其中包含：
...
#### Step 1.5:  **创建 和生成 md 工作区文档** **`exp_i.md`** 

**项目开始前必须检查`\Exp_i\report\exp_i.md`** **是否创建，没有创建则项目不能开始！！！**

**这一步是核心**： 将 docx 模板的结构和内容转换注入 `\Exp_i\report\exp_i.md`，作为本次实验的**可编辑工作文档，不可更改原docx 模板内容**。
...
用户确认满意后，开始将 `exp_i.md` 中的内容填充到 Word 模板。在编写Word过程中， **不可以更改模板文件原有的格式或内容** 。
